def handel_upto_99(number):
    ...
    if number in predef.keys():
        ...
        return predef[int((number/10))*10]+" "+predef[number%10]
...
def return_bigdigit(number,devideby):
    ...
    if devideby in predef.keys():
        return predef[int(number/devideby)]+" "+predef[devideby]
    else:
        devideby/=10
        return handel_upto_99(int(number/devideby))+" "+predef[devideby]
    ...
    if number<100:
        ...
        while number>=100:
            ...
            if devideby not in dev:
                number=number-((devideby/10)*(int(number/(devideby/10))))
            number=number-devideby*(int(number/devideby))
